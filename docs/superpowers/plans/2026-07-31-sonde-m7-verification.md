...
**Files:**
- Create: `fuzz/Cargo.toml`, `fuzz/fuzz_targets/{interpret,event_log,canonical_json,manifest,ipc}.rs`
...
**Acceptance criteria:**
- **AC-7.7** A fuzz target exists for every function that consumes untrusted bytes: interpretation, event log parsing, canonical JSON, manifest loading, and the `packetd` IPC protocol.
- **AC-7.8** Each target survives 120 seconds with no crash, hang, or OOM.
- **AC-7.9** `interpret`'s fuzz target asserts every returned `matched_span` is a valid index range into the input.
- **AC-7.10** CI runs fuzz targets on every pull request with a cached corpus.

...
>
> **SDK:** `rmcp` 3.1.0 (released 2026-07-31), official, MSRV **1.88** and edition 2024. **This paragraph then drew the wrong conclusion, and Task 4 corrected it: `bathy-mcp` is a 1.95-tier crate and belongs in `msrv-bathy-store`, not `msrv`.** The SDK's floor is a *lower bound* on this crate's floor, not its value — `bathy-mcp` is an adapter over the engine, so it depends on `bathy-engine` → `bathy-store` → `libsqlite3-sys`, whose build script needs `cfg_select!`. Verified against real toolchains in both directions before the job was edited: `cargo +1.88 check -p bathy-mcp` fails with "bathy-mcp@0.1.0 requires rustc 1.95", `cargo +1.95 check -p bathy-mcp --all-targets` passes. The instruction to **verify at creation rather than assume it** was the right instruction and is what caught this (M3 found `bathy-plan` had sat unverified against its claimed floor since creation, and M4 reintroduced the same defect with `crates/bathy`). Caveat to record in `docs/protocol-notes.md`: the official MCP blog rates the Rust SDK's `2026-07-28` support **beta**, against Tier-1 for TypeScript/Python/Go/C#, while the SDK's own README says "stable". Trust the blog. Budget for rough edges specifically around MRTR, `server/discover` and the Tasks extension, and if the SDK cannot express something the spec requires, **report it as a finding rather than working around it silently.**
...
**The first two arms are conditional, and that is the whole task.** Absent-then-present is an `EndpointAppeared` only if the earlier scan proved it looked; present-then-absent is an `EndpointDisappeared` only if the later one did. "Proved it looked" is `Terminal::Completed` on both sides -- `bathy-engine` emits `scan.completed` on exactly one path, plan exhaustion, so a refused (`Denied`), failed or budget-exhausted (`Failed`) or cancelled/still-running (`None`) scan is a scan that stopped early -- **and** the same `plan_hash` on both sides, because two completed scans of different plans need not have looked at the same endpoints. Anything else goes to `undetermined`. An endpoint present in *both* folds is always decidable, whatever the terminals say: both scans demonstrably observed it.

**`plan_hash` is a proxy for coverage that is strictly finer than coverage, and "different plans" understates the rule's reach.** `bathy-plan` deliberately includes `budgets` in the hash — its own `plan_hash_changes_when_budgets_change`, and its module doc: "reusing a key with a larger budget must be a conflict". So an operator who lowers a rate limit, raises a packet ceiling or extends a runtime cap between Monday and Tuesday gets `undecidable: CoverageDiffers`, zero appearance/disappearance classifications, and every one-sided endpoint in `undetermined` — on two scans that looked at exactly the same endpoints. Tuning a budget is a routine operational act; changing a port range is not, so this is the commoner input. Read "different plans" as "did not run the same *authorization*", never as "did not cover the same ports". Ruled acceptable for v0.1 in the M5 Task 2 review — the direction is safe, nothing is hidden, and AC-5.34 asks only that no phantom change be produced — and **AC-5.37 carries the fix**.
...
- Produces: the `bathy` binary, with subcommands `scope validate`, `scan preview`, `scan start`, `scan status`, `scan events`, `scan cancel`, `scan resume`, `result query`, `result diff`, `evidence get`, `explain`.
  ...
  **Six further corrections to this line and to Step 3. The first four were made during Task 3; #5 and #6, and the residuals recorded inside #2 and #3, were made in the Task 3 fix round after review. All are flagged as plan edits.**
  ...
  3. **A usage error must not exit 2.** `clap`'s own `Error::exit` uses status 2, which this plan assigns to *policy denial* — so a typo would be indistinguishable from an authorization refusal to the agent this exit-code table exists for. The CLI therefore parses with `try_parse` and maps usage errors to 1 (operational), and `--help`/`--version` to 0. Any future argument parser has to make the same choice; it is a property of the exit-code table, not of `clap`.
     ...
     **Residual found by the Task 3 review and fixed in the fix round: "usage errors map to 1" was implemented as a three-kind `matches!`, and it swept `DisplayHelpOnMissingArgumentOrSubcommand` in with `--help`.** `bathy --json scan` therefore exited **0 with zero bytes on stdout**, which the agent this table exists for can only read as "the scan found nothing" — while `bathy --json`, the same mistake with an argument present, exited 1. **The rule is stronger than the original wording and is now stated as such: a malformed invocation must never be indistinguishable from an empty result.** Every one of `clap`'s seventeen `ErrorKind`s is mapped deliberately, `ErrorKind` is `#[non_exhaustive]` so the mandatory wildcard resolves to a usage error, and the enumeration caught two more of the same class (`Io`, `Format` — kinds `clap` does not call errors, which nonetheless delivered nothing). The trap worth recording for any future parser: `clap` groups its kinds by *its* notion of severity, and adopting that grouping is what produced the defect.
  5. **Under `--json`, `--help` and `--version` are JSON documents, not prose.** AC-5.10 says stdout carries line-delimited JSON and nothing else; `--json --help` put 1,273 bytes of prose there. Both available readings were defensible — exempt `--help` in the criterion, or move the prose to stderr — and a third was better: emit one document carrying the help text *and the exit-code table as data* (rendered from `ExitCode::TABLE`, so AC-5.11's table cannot drift from its machine-readable form). No exception is carved out of AC-5.10, nothing an agent might want is moved to the diagnostic channel, and the invariant a caller can actually branch on holds without qualification: **under `--json`, an exit-0 run always leaves at least one parseable line on stdout.** Human mode is unchanged — prose on stdout, exit 0. Decided in the Task 3 fix round rather than by accident, which is what the review asked for.
  6. **`scan events --follow` has an idle timeout.** Its only stopping condition was a terminal event, and a scan whose process was killed never writes one, so a follower waited forever — and an agent that shelled out has no interrupt to send. `--idle-timeout-seconds` (default 300, `0` to wait forever) bounds it, and giving up is exit 1 with `follow_idle_timeout`, not a success with a truncated event stream.
...
| `scan.preview` | `{ manifest_path, request: ScanRequestSpec }` | `{ plan_hash, estimated_targets, estimated_probes, policy_decision, reason_code?, estimated_runtime_seconds }` |
| `scan.start` | `{ manifest_path, request: ScanRequestSpec }` | `TaskHandle` (in `structuredContent`) · or `{ policy_decision: "denied", reason_code, detail }` · or an MRTR `InputRequiredResult` (`resultType: "input_required"`) carrying an embedded `elicitation/create` plus an authenticated `requestState` |
...
| `evidence.get` | `{ digest, max_bytes? }` | `{ bytes_hex, length, truncated }` |
...
**Two further corrections to this table, made during Task 4.**

1. **The request input is a `ScanRequestSpec`, not a `ScanRequest`.** `ScanRequest` carries `authorization_scope_id`, so a table entry reading `request: ScanRequest` puts a **scope id in a published input schema** -- which AC-5.38 forbids in the same breath as `manifest_json` -- and leaves the paragraph above having to specify a validation step ("refuses rather than reconciles if the caller supplied a different one") where an absence does the job. `ScanRequestSpec` is every field of a request except that one, plus the three optional budget overrides `--max-packets`/`--max-runtime-seconds`/`--max-packets-per-second` already take; the server fills the scope identity from the manifest it loaded. There is then nothing to reconcile and nothing to refuse, which is the same fix `AuthorizedScan` and required-`--scope` apply at the two layers either side of this one: **make the unauthorized state unconstructible rather than checked.**
2. **`evidence.get` returns `bytes_hex`, not base64.** This table said base64; Task 3 shipped `bytes_hex` from `bathy evidence get --json`, with a stated reason (no encoder dependency for one field, a human can compare a short banner against a packet capture, and the digest that names the bytes is written the same way). **Two encodings of one contract is worse than either encoding**, and the shipped surface is the one an operator already has, so the tool matches the command rather than the other way round. The field name says which encoding it is, which `bytes` did not.

**And one thing this table asked for that the CLI could not do**, found the same way Task 3's fix round found the `scope_id` problem and fixed at the cause rather than dropped: `scope.validate` takes `{ manifest_path, targets[] }` and `bathy scope validate` took only `--scope`, so the tool surface could answer a question the command surface could not. `bathy scope validate` gained `--targets`, and both surfaces now render the *same* `ScopeValidateOutput` from the *same* function -- two implementations of one authorization question is how two surfaces come to disagree about authorization. The same sweep added `estimated_runtime_seconds` to `bathy scan preview`, which this table named and Task 3 had not implemented.
...
- **AC-5.29** Every tool carries explicit annotations, and a test must prove they are *set* rather than that the effective posture happens to be safe -- the spec's default for an unannotated tool is already maximally cautious, so a safe posture can be held by accident and lost by a later edit without anything failing.

  **Corrected in Task 4: three tools are not read-only, not one.** This criterion read "`scan.start` is `readOnlyHint: false, destructiveHint: true, openWorldHint: true`; the ten read-only tools are `readOnlyHint: true`", which is wrong about two of the ten and inherited the error from the research note's own list (that list named *eight* read-only tools -- `scope.validate`, `scan.preview`, `scan.status`, `scan.events`, `result.query`, `result.diff`, `evidence.get`, `fingerprint.explain` -- and the criterion turned eight into "the other ten" without checking which two had been left out).

  - **`scan.resume` emits packets.** It continues a scan from its cursor by building a `Scheduler` and running it, exactly as `scan.start` does -- `bathy scan resume` reaches `run_to_completion` on the same path. Annotating it `readOnlyHint: true` would tell a client that a tool which puts real traffic on a third party's network does not modify its environment. Per the Global Constraint on the README, **understating what we put on someone else's wire is not a safe error**, and this is that error in the one vocabulary a client is specifically told to use for risk. It is `readOnlyHint: false, destructiveHint: true, openWorldHint: true, idempotentHint: false` -- not idempotent, because a second resume can send more packets.
  - **`scan.cancel` changes a running scan's state.** It emits nothing and it is not destructive (`scan.resume` undoes it), but it is not a read: `readOnlyHint: false, destructiveHint: false, openWorldHint: false, idempotentHint: true`.
  - **`scan.start`** is `readOnlyHint: false, destructiveHint: true, openWorldHint: true`, and `idempotentHint: true` -- repeating it with the same key and plan returns the original scan and starts nothing, which is what the hint means.
  - The remaining **eight** are `readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false`.

  All four hints are set on all eleven. Closed by `the_three_tools_that_change_something_are_not_advertised_as_reads` and `every_advertised_tool_declares_an_output_schema_and_explicit_annotations`, both asserted against the advertised tool over a real transport.
...
- **AC-5.37** **A budget change alone must not be presented to an agent as a coverage change.** `Undecidable::CoverageDiffers` fires whenever `plan_hash` differs, and `plan_hash` covers budgets as well as targets and ports, so two scans of identical coverage are reported as incomparable if a rate limit moved between them (reproduced in the M5 Task 2 review: same coverage unit-for-unit, `maximum_packets_per_second` 10_000 → 5_000, `plan_hash` differs). Two things close this criterion, and the first is already done:
  1. The published `description` of `Undecidable::CoverageDiffers` says plainly that a budget change alone triggers it. Closed by `the_coverage_differs_reason_says_a_budget_change_alone_triggers_it` (M5 Task 2 fix round).
...
**Acceptance criteria:**
- **AC-5.24** A complete authorized inventory — validate, preview, start, poll to terminal, query, fetch evidence — completes in **six typed calls plus however many polls the scan takes**, which is ten or fewer against the lab. Closed by `an_agent_completes_an_authorized_inventory_in_ten_typed_calls`, which takes nine and fails rather than exceeding ten. **The ten is conditional and the six is not.** There is no protocol-level way to be told a scan has finished — a held response stream is what clients and intermediaries time out (`docs/protocol-notes.md`) — so an agent asks, and asking costs a call. How many times it must ask is a function of how long the scan takes against a fixed backoff, which is a property of the machine: re-paced six times slower, this workflow takes eleven. So the criterion is closed on two constants asserted separately — `TYPED_CALLS_BESIDES_POLLING = 6` **exactly**, which is what "the surface is sufficient" actually means and is machine-independent, and `CALL_BUDGET = 10` as a **bound**, which is not. A budget failure names the machine as the cause and forbids raising the ten silently; it is not evidence about the surface. The same condition is stated in the opening paragraph of `docs/examples/agent-inventory-workflow.md`, and this sentence exists because the wave that wrote that error message told the reader to restate it here and the plan file was not in that diff. **Every call is made over the stdio transport and every decision is taken on a typed field of `structuredContent`**: the text mirror is discarded by the client before the workflow can see it, so the surface is proved *sufficient* rather than merely present. The workflow contains a refusal — an unauthorized target, denied with a stable `reason_code` and a typed `out_of_scope` the agent narrows its own request with — because an inventory that only ever succeeds demonstrates none of the safety properties.

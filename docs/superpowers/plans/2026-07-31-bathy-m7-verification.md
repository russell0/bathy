...
- Modify: `crates/bathy-probe/src/probes/tls.rs` (see plan edit #5)
  ...
  **Five corrections to this task, made during Task 1 and flagged as plan edits. The first two are defects in the acceptance surface itself: one AC could not fail, and the checked-in oracle was wrong.**
  ...
  1. **AC-7.4's drafted test is a decoration test.** `assert!(!fold.hosts_up.contains(&ip))` could not fail on the tree it was written against: `Scheduler` had no call to `discover_host` (unprivileged ICMP is impossible, so host discovery shipped with `packetd` in M6 — the overview's Gap Register says so), which meant `ScanFold::hosts_up` was empty after *every* scan. As written the criterion passes against a scanner that reports the whole subnet live. The property that is real either way: every endpoint on an address with no host must be `Filtered` — never `Open` (a service we invented) and never `Closed` (a claim that an RST arrived, which is itself evidence a host is there). The shipped test asserts that, and separately asserts `hosts_up` **is** empty. **M6's AC-6.20 fix wave then wired the discovery phase**, gated on `Objective::HostInventory`; the lab scan requests `Objective::InventoryExposedServices`, so the second assertion did not go red — it became the objective gate's lab-side control, which is the stronger form this edit was written to force. AC-7.4 is restated below.
...
- **AC-7.4** Addresses with no host are never reported `Open` or `Closed`; every endpoint on one is `Filtered`. *Restated — see plan edit #1.* "Reported down" was not assertable when this was written: `hosts_up` was empty after every scan because host discovery shipped with `packetd` in M6, so the original wording named a test that could not fail. Since M6's AC-6.20 fix wave it is assertable, but only for `--objective host-inventory`; the lab scan uses `inventory-exposed-services`, so the restated criterion is still the right one and the `hosts_up` assertion beside it is now the objective gate's control.
...
- Modify: this file and the overview — see plan edit #1
  ...
  **Four corrections to this task, made during Task 4 and flagged as plan edits. The first is a defect in the acceptance surface itself: the plan document that states AC-7.17 violated it.**
  ...
  1. **AC-7.17's own target contained the thing AC-7.17 forbids.** These plans live under `docs/`, which is precisely the tree the criterion says must not name an individual, and Task 6's Step 1 sketch spelled a name out three times — in the research-artifact filename filter, in the positioning grep, and in a test fixture — with a fourth in the overview's Pre-Publication Gates. The sketch also contradicted the plan's *own* AC-7.25, which says personal identifiers live only in the git-ignored `.publish-deny` and never in the checker's source. All four sites now read the register instead, and Task 6 inherits `denied_names()` rather than a literal list. The name no longer appears anywhere in this repository, which is the property AC-7.17 exists to keep and which a `grep -ri` can now confirm in one second.
     ...
     **Corrected in Task 6, and the correction is the more important half.** That last sentence was wrong, and it was wrong in the way this repository keeps being wrong: `grep -ri` reads the **working tree**, and what a `git push` publishes is the **history**. The first real run of `publish-check` — which reads every blob in the object database, not the checkout — found both identifiers still present in commit `ab291b8` (*"docs: sonde v0.1 implementation plans, seven milestones, 185 acceptance criteria"*), at line 391 of the plan file this one was renamed from, in exactly the hardcoded pattern list AC-7.25 forbids. That commit is an ancestor of `m1-m3-implementation` **and of `main`**, and `main` is already pushed to `origin`.

     So Task 4's fix was a working-tree fix for a history-level defect, and the sentence that declared it complete named the tool that could not see the rest of it. Nothing about the working tree needs changing; **the repository still cannot be published until the history is rewritten**, and that is recorded as a blocking item under the overview's Pre-Publication Gates rather than left as a green tick here. The general lesson is worth more than the instance: *a claim about "this repository" that is verified by reading the checkout is a claim about the checkout.*
...
`docs/design-paper.md`, working title *Network Discovery After the CLI: Designing a Deterministic Planner for Autonomous Software Agents*. Sections: the integration gap between XML/CLI and typed tool calling; the evidence and provenance model; why planning is deterministic and observation is not; three-layer scope enforcement (drafted as "two-layer"; M6 Task 5 added the third, inside `bathy-packetd`); the plugin sandboxing plan; measurements; **limitations and threats to validity**; a clean-room attestation.

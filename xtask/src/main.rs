...
/// `gen-ports`: regenerate `crates/bathy-plan/data/ports/top-100.txt` and
/// `crates/bathy-plan/data/ports/common-1000.txt` from a local copy of the IANA *Service Name
/// and Transport Protocol Port Number Registry* CSV.
///
/// # Clean-room note
///
/// This reads only the file named by `--registry`, which the caller must
/// have fetched from IANA (see `crates/bathy-plan/data/ports/README.md`). It never reads,
/// searches for, or consults any Nmap data file, installed or otherwise.
///
/// # The ranking heuristic
///
/// Exactly the two-step ranking documented in `crates/bathy-plan/data/ports/README.md`:
///
...
fn gen_ports() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut out_dir = PathBuf::from("crates/bathy-plan/data/ports");
...
fn write_dataset(path: &Path, ports: &[u16]) -> Result<(), String> {
    let mut body = String::from(
        "# Generated by `cargo run -p xtask -- gen-ports`. Do not hand-edit --\n\
         # see crates/bathy-plan/data/ports/README.md for provenance and regeneration.\n",
    );
    for p in ports {

...
> inconclusive — is wired into the scheduler: `--objective host-inventory`
> runs a discovery phase and emits `host.discovered` events, so `hosts_up` is
> populated for those scans and empty for every other objective. Its ICMP
> half needs a `bathy-packetd` no shipped surface can reach, so from the
> command line the deciding method is always a TCP one.
>
...
empty even though the host is plainly up: host discovery runs only for
`--objective host-inventory`, and this scan used the default objective
(`inventory-exposed-services`), which runs no discovery phase at all. Both are
...
| `bathy-engine` | The scheduler: budget-governed, rate-limited, cancellable, resumable execution of a `ScanPlan` over real unprivileged TCP connect probes, with scope identity, manifest expiry, and per-target authorization all checked directly on the actual emission path. Drives service identification on top of that — up to `intensity` further paced, budgeted, scope-checked connections per open port, stopping at the first response a rule recognizes — and stores the evidence bytes *before* emitting the event that cites them. Runs a host-discovery phase before that unit loop for a scan requesting `--objective host-inventory`, and for no other objective: it asks the manifest per address, charges the ledger the worst case before a packet leaves, and emits one `host.discovered` naming the method that decided — the combined method, which asks `bathy-packetd` for an ICMP echo first and falls back to the unprivileged TCP one when that is inconclusive. That phase is the only thing in the workspace that constructs a `host.discovered` event, and a resumed scan does not repeat it. |

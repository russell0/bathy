...
| `bathy-packetd` | The privileged helper, so far only the line protocol it will be spoken to over. It opens no socket, holds no capability and emits no packet on this branch — it is `#![forbid(unsafe_code)]` like every other crate here. What it does today is refuse: nothing but an `init` may arrive first, an `init` with an empty allowlist is refused rather than read as "allow everything", a second `init` cannot widen the session's scope, and malformed or oversized input ends the session instead of being resynchronized past. Every one of those refusals is final — the session answers `fatal` to everything afterwards, including a valid `init`. Raw sockets, capability dropping and SYN/ICMP probing are the rest of Milestone 6. |
